...
from .sensor import SIGNAL_TOKEN_USAGE_UPDATED
...
class ExtendedOpenAIAgentEntity(
    ...
    async def _async_handle_message(
        ...
        try:
            ...
            await self.hass.services.async_call(
                ...
                "create",
                {
                    "title": "Kimi Assist error",
                    "message": f"{err}",
                    "notification_id": "kimi_assist_error",
                },
            )
            intent_response = intent.IntentResponse(language=user_input.language)
            intent_response.async_set_error(
                intent.IntentResponseErrorCode.UNKNOWN,
                "Sorry, I had a problem talking to Kimi. Please check the notification for details.",
            )
            return conversation.ConversationResult(
                response=intent_response, conversation_id=conversation_id
            )
            ...
            intent_response.async_set_error(
                intent.IntentResponseErrorCode.UNKNOWN,
                "Something went wrong with the voice assistant. Please check the notification.",
            )
            return conversation.ConversationResult(
                response=intent_response, conversation_id=conversation_id
            )
    ...
    async def query(
        ...
        # Disable Kimi/Moonshot reasoning content to reduce token usage and latency.
        # If the upstream endpoint does not support the thinking parameter, it will
        # return an error and this can be reverted.
        ...
        if response.usage:
            ...
            async_dispatcher_send(self.hass, SIGNAL_TOKEN_USAGE_UPDATED)
